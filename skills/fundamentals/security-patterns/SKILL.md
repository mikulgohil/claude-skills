...
description: Application security patterns — authentication, defense-in-depth, input validation, OWASP Top 10, secrets handling, PII masking, and LLM safety. Use when handling auth, untrusted input, or sensitive data.
author: Mikul Gohil
version: 1.0.0
license: MIT
tags: [security, owasp, auth, input-validation, pii, llm-safety]
...
Security hooks shipped by the project are **managed defaults** — users can disable them but are warned. Enterprise admins can lock settings via managed profiles.
